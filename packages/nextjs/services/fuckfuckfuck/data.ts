...
const supportedDataEntryTypes = new Set<DataSchemaEntryType>([...SUPPORTED_TYPES, ...SUPPORTED_MIME_TYPES]);

const searchableDataEntryTypes = new Set<SearchableSchemaEntryType>([...supportedDataEntryTypes, ...LEGACY_TYPES]);
...
export const ensureSearchableDataSchemaIsValid = (schema: SearchableDataSchema) => {
  if (schema === undefined) {
...
export const extractDataSchema = async (data: DataObject): Promise<DataSchema> => {
...
export const createZipFromObject = (obj: unknown): Promise<Uint8Array> => {
  ...
  const createFileOrDirectory = (key: string, value: unknown, path: string): void => {
...
export const reverseSafeSchema = function (schema?: Array<Record<"id", string>>) {
  if (!schema) {
    return {};
  }
